...
QString MsXmlXsltProcessor::ProcessXslt(const QString &xmlData, const QString &xsltData)
{
    ...
    if (xml->querySubObject("parseError")->property("errorCode").toInt() != 0)
    {
        ...
        lastError = "Xml file parse error" + xml->querySubObject("parseError")->property("reason").toString();
    ...
    if (xslt->querySubObject("parseError")->property("errorCode").toInt() != 0)
    {
        ...
        lastError = "Xml file parse error" + xslt->querySubObject("parseError")->property("reason").toString();
    ...
    {
        exceptionFlag = false;
        lastError = "Xml file parse error";
        return QString();
    }
